...
**Where the edge shows up.** Baton's edge tracks stakes, not size: a wash on low-stakes work (where it also costs more), real catches when being wrong is expensive. The driver is not task size; it is that independent verification and running for real catch defects a green suite passes.

![Baton's edge over a bare model, plotted against stakes. Four washed benches cluster at no-edge on low-stakes work and cost more; two field runs sit well above at high stakes (production, real infrastructure), where independent review and running for real caught defects a green test suite passed. Real points only, no fitted trend; the space between the ends is unmeasured.](docs/evidence.png)

Real points only, no fitted trend. The only data points are the four washed benches (no edge on low-stakes correctness, at higher cost) and two field runs (N=2, private code) where independent review and running for real caught defects a green suite passed; the space between the ends is unmeasured. The value is amplify-not-generate, and cost versus a careful engineer plus one sharp review is unmeasured. The chart regenerates from [`docs/make-evidence-chart.R`](docs/make-evidence-chart.R); detail in [`docs/field-notes.md`](docs/field-notes.md) and [`docs/research-basis.md`](docs/research-basis.md).
