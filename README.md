...
**When it helps.** On small, self-contained tasks, Baton does no better than plain AI and costs a little more. On real development work, a separate review and real-world testing caught bugs the automated tests had missed. What makes the difference is the extra checking, not how big the task is (a bigger but self-contained test still showed no gain).

![Chart titled "When Baton helps, and when it doesn't." On the left, four small tests sit at "no difference" from plain AI and cost more. On the right, two real projects sit well above, where a separate review and real-world testing caught bugs the tests had missed. The middle is marked untested. Only real results are shown, with no predicted trend line.](docs/evidence.png)

Plain version: on basic tasks Baton matches plain AI and costs a bit more; on the two real projects we ran (private code), a separate review and real-world testing caught bugs the tests had passed. We have not tested the middle, and the gain comes from the extra checking, not the size of the work. Baton makes good practice consistent; it does not make the AI smarter, and whether it is cheaper than a careful engineer plus one good review is still untested. Chart from [`docs/make-evidence-chart.R`](docs/make-evidence-chart.R); more in [`docs/field-notes.md`](docs/field-notes.md) and [`docs/research-basis.md`](docs/research-basis.md).
