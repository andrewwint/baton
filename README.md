...
**The shape of it.** Baton's edge tracks development complexity: a wash on trivial, low-stakes work, real catches on complex efforts.

![Baton's edge over a bare model vs development complexity. Four washed benches cluster at no-edge on trivial, low-stakes work (and cost more); two field runs sit well above on complex development efforts, where independent review and live deploys caught defects a green suite passed. The connecting trend curve is illustrative, not a measurement.](docs/evidence.png)

Illustrative, not a plotted measurement. The only real data points are the four washed benches (no edge on trivial, low-stakes correctness, at higher cost) and two field runs (N=2, private code) where independent review and running-for-real caught defects a green suite passed. The value is amplify-not-generate, and cost versus a careful engineer plus one sharp review is unmeasured. The chart regenerates from [`docs/make-evidence-chart.R`](docs/make-evidence-chart.R); detail in [`docs/field-notes.md`](docs/field-notes.md) and [`docs/research-basis.md`](docs/research-basis.md).
