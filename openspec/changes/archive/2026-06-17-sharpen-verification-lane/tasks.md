...
- [x] 1.1 Under "Look past a green suite," add a check: scrutinize any altered or removed tests in the
...
- [x] 1.2 Add a check: before escalating a failing verification, root-cause it to a real defect versus
...
- [x] 2.1 In the verify step (loop step 5) or Required Checks, add one line directing the manager to
...
- [x] 3.1 `openspec validate sharpen-verification-lane --strict` passes
- [x] 3.2 `npm run smoke` green from `.claude/skills/baton/runtime` (offline; no runtime/behavioral
