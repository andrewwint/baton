...
- [x] 2.3 Decision (descoped): bench wiring is deferred to a follow-on change. The analyzer runs standalone today, and the proposal already scopes bench integration as follow-on. The efficacy signal ("did discovery change the result?") is intentionally not pursued, since it drifts back toward the end-state correctness question the bench washed on four times.
